...
function nextFact(tables: number[], last: Fact | null, focus?: Fact[]): Fact {
  const pool =
...
export function MultiplicationTables() {
  const [tables, setTables] = useState<number[]>(EASY);
  const [mode, setMode] = useState<Mode>("practice");
  const [phase, setPhase] = useState<Phase>("setup");
  const [showChart, setShowChart] = useState(false);
  const [fact, setFact] = useState<Fact>({ a: 2, b: 5 });
    ...
    <div>
      ...
      {showChart ? (
        <div className="no-print mt-4">
          <MultiplicationChart highlightTables={tables} />
        </div>
      ) : null}

      <div className="mt-6 hidden print-only">
        <h2 className="mb-3 font-display text-2xl">Multiplication chart (1–12)</h2>
        <MultiplicationChart />
      </div>
    </div>
  );
...
function SetupPanel({
  ...
  onPreset: (tables: number[]) => void;
  onMode: (mode: Mode) => void;
  onStart: () => void;
}) {
  return (
    <div>
      <h2 className="font-display text-2xl text-ink">Choose tables</h2>
      <p className="mt-1 text-sm text-ink-muted">
        Tap the facts you want. Presets get you started in one tap.
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        <PresetButton
          label="Easy 1 / 2 / 5 / 10"
          active={sameSet(tables, EASY)}
          onClick={() => onPreset(EASY)}
        />
        <PresetButton
          label="Hard 6 / 7 / 8 / 9 / 12"
          active={sameSet(tables, HARD)}
          onClick={() => onPreset(HARD)}
        />
        <PresetButton
          label="All 1–12"
          active={sameSet(tables, ALL_TABLES)}
          onClick={() => onPreset(ALL_TABLES)}
        />
      </div>
      <div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-6">
        {ALL_TABLES.map((n) => {
          const selected = tables.includes(n);
          return (
            <button
              key={n}
              type="button"
