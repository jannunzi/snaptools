...
function TrebleClef() {
  // Native path is 15.186 × 40.768. The G spiral's inner curl sits near y=25.5,
  // which must land on G4 (second staff line from the bottom).
  const scale = 2.9;
  const gLineLocal = 25.5;
  const x = 28;
  const y = stepToY(4) - gLineLocal * scale;

  return (
    <g transform={`translate(${x} ${y}) scale(${scale})`} aria-hidden>
      <path d={TREBLE_CLEF_PATH} fill="currentColor" fillRule="evenodd" />
    </g>
  );
}
