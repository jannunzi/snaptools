...
function speakWord(word: string, locale: string) {
...
export function SpellingPractice() {
  ...
  const submit = useCallback(() => {
    ...
    advanceTimerRef.current = window.setTimeout(() => {
      ...
      setFeedback(null);
      busyRef.current = false;
      speakWord(upcoming, list.locale);
      inputRef.current?.focus();
    }, correct ? 900 : 1800);
  }, [input, list, phase, word]);
      ...
      {phase === "setup" ? (
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            Speech uses your browser’s voices. Headphones help. The word stays
            hidden until you check your spelling.
          </p>
          ...
          {!speechReady ? (
            <p className="mt-3 text-sm text-ink-muted">
              Loading voices… tap Start when you are ready.
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      {phase === "playing" ? (
        <div>
          ...
          <button
            type="button"
            onClick={() => hear()}
            className="mt-5 inline-flex min-h-12 items-center justify-center rounded-xl border-2 border-secondary bg-secondary-soft px-4 text-sm font-semibold text-secondary"
          >
            Hear word again
          </button>
