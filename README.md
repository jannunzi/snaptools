...
- [History Timeline](/tools/history-timeline) — horizontal world history (past left, future right) with parallel lanes. Add, remove, or drag-reorder rows, set color in a lane’s config, zoom millennia to days, and fill missing spans with Grok. Lanes, colors, zoom, and scroll restore on reload. Seeded events show on first paint; MongoDB caches category + time window + granularity.
